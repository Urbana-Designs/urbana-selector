...
(function ($) {
	...
	$(document).ready(function () {
		...
		if ($('#chart-interactions-months').length) {
			initDashboardCharts();
		}
	...
	function initDataViewsEnhancements() {
		...
		var isDashboardPage = $body.hasClass('toplevel_page_urbana-main') || $body.hasClass('urbana_page_urbana-guild-ledger') || $body.hasClass('urbana-main_page_urbana-guild-ledger');
	...
	function renderRows(items) {
		...
		items.forEach(function (item) {
			...
			var typeLabel = formatInteractionTypeLabel(item.interaction_type);
		...
		fetch(urbanaLedgerData.restUrl + 'urbana-guild-ledger/v1/stats', { headers: { 'X-WP-Nonce': urbanaLedgerData.nonce } })
			...
			.then(function (data) {
				...
				if (ctx2) {
					new Chart(ctx2, {
						type: 'bar',
						data: { labels: typeLabels.map(function (t) { return formatInteractionTypeLabel(t); }), datasets: [{ label: 'Interactions', data: typeCounts, backgroundColor: ['#60a5fa', '#a78bfa', '#34d399', '#fbbf24', '#94a3b8'] }] },
						options: { responsive: true, maintainAspectRatio: false }
					});
				}
