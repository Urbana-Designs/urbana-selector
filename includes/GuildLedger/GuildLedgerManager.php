...
// Build meta_query conditionally only when needed to avoid unnecessary slow queries
     ...
     * Add admin menu
     * 
     * consider adding all sub-pages in the same way as here and remove them from the Urbana\Admin\AdminInit.php - NM
     */
    ...
    public function enqueue_admin_scripts($hook) {
        global $post_type;
        $is_dashboard_page = 'urbana-main_page_urbana-guild-ledger' === $hook;
        
        if ($post_type === self::POST_TYPE || $is_dashboard_page) {
            // Enqueue WordPress DataViews dependencies
            wp_enqueue_style('wp-components');
            
            // Enqueue custom admin styles and scripts
            wp_enqueue_style('urbana-guild-ledger-admin', plugins_url('urbana-guild-ledger-admin.css', __FILE__), array('wp-components'), URBANA_VERSION);
            wp_enqueue_script('urbana-guild-ledger-admin', plugins_url('urbana-guild-ledger-admin.js', __FILE__), array('jquery', 'wp-element', 'wp-components', 'wp-data'), URBANA_VERSION, true);
    ...
    /**
     * Ensure default lead status terms exist.
     */
